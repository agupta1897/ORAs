...
def tf_ssim(img1, img2, cs_map=False, mean_metric=True, size=11, sigma=1.5):
    ...
    K1 = 0.01
    K2 = 0.03
    L = 3  # depth of image (255 in case the image has a differnt scale)
    C1 = (K1*L)**2
    C2 = (K2*L)**2
...
def MS_SSIM_l1_loss(y_true, y_pred):
    loss_ms_ssim = 1-tf_ms_ssim(y_true,y_pred)
    # loss_l1= tf.abs(y_true-y_pred)
    return loss_ms_ssim+losses.mean_absolute_error(y_true, y_pred)
